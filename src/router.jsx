...
import team from './assets/team.jpg'//not used now
import landingMainImg from './assets/xx-min.jpg' 
import './routercss.css';
import NavBar from './components/NavBar';
import { auth, firestore } from './config'; // Import Firebase auth and Firestore
...
import VSModeQuizInterface from './pages/vsModeQuizInterface';
import TeamMode from './pages/vsMode';
  ...
  //------------------------------------------------------------DARK MODE FUNCTIONS AND SH#@$------------------------------------------------------------------------
    ...
    const fetchLevels = useCallback(async () => {
        try {
            ...
            if (user && user.userLevel && levelsData && levelsData[user.userLevel]) {
                setSelectedLevelData(levelsData[user.userLevel].subjects);
            }else if(user && user.userLevel===undefined && levelsData && levelsData[user.userLevel]===undefined){
                setSelectedLevel(levelsArray[0]);
                setSelectedLevelData(levelsData[selectedLevel].subjects);
            }else {
                console.log(`No data found for the selected level: ${user?.userLevel}`);
            }
        } catch (error) {
            console.error('Error fetching levels:', error);
            toast.error('Error fetching levels from database')
        }
    }, [databaseSnapshot, user]);
    ...
    const navBarData = {
      user,realUser,setUser,toggleDarkMode,isDarkMode
    }

    return (
        <>
            {loading && (
                <div className="loading-overlay z-50 w-screen h-screen absolute bg-white dark:bg-gray-900 ">
            ...
            {/* {user && <NavBar userClone={user} realUser={realUser} setUser={setUser} />} Show NavBar if user is authenticated */}
            
            <Routes>
                <Route path='/signup' element={<SignupPage realUser={realUser}/>} />
                <Route path='/login' element={<LoginPage setLoad={setThatLoader} realUser={realUser}/>} />
                <Route path='/' element={<Landing user={user} loading={loading}/>} />
                
                <Route path='/terms' element={<TermsPage navBarData={navBarData} />} />
                        <Route path='/privacy' element={<PrivacyPolicyPage navBarData={navBarData} />} />

                {/* Protected routes */}
                {user !== null && (
                    <>
                        <Route path='/dashboard' element={<Dashboard setLoading={setLoading} user={user} levels={levels} setSelectedLevel={setSelectedLevel} selectedLevel={selectedLevel} setSelectedLevelData={setSelectedLevelData} levelsData={levelsData} navBarData={navBarData}/>} />
                        <Route path='/quiz' element={<QuizPage user={user} selectedLevelData={selectedLevelData} navBarData={navBarData}/>} />
                        <Route path='/user/profile' element={<UserProfile navBarData={navBarData}/>} />
                        <Route path='/quiz/interface' element={<QuizInterface loading={loading} setLoading={setLoading} user={user} />} />
                        <Route path='/quiz/result' element={<QuizResult user={user} realUser={realUser}/>} />
                        <Route path='/user/statistics' element={<UserStatistics user={user} navBarData={navBarData} setLoading={setLoading}  />} />
                        <Route path='/user/settings' element={<Settings navBarData={navBarData} user={user} setUser={setUser} setLoading={setLoading} levels={levels} setSelectedLevel={setSelectedLevel} selectedLevel={selectedLevel} setSelectedLevelData={setSelectedLevelData} levelsData={levelsData}/>} />
                        <Route path='/leaderboards' element={<Leaderboards user={user} navBarData={navBarData}/>} />
                        <Route path='/help' element={<HelpPage navBarData={navBarData} />} /> 
                        <Route path='/events' element={<Events navBarData={navBarData} />} /> 
                        <Route path='/user-progress' element={<Progress navBarData={navBarData} />} /> 
                         <Route path='/quiz/vs-mode' element={<TeamMode navBarData={navBarData} />} /> 
                        {/*<Route path='/quiz/vs-mode/interface' element={<VSModeQuizInterface navBarData={navBarData} />} />  */}
                        {user.status === 'admin' && (
                            <>
                            <Route path='/dashboard/admin' element={<AdminDashboard navBarData={navBarData} />} />
                            {/* <Route path='/admin/user-management' element={<AdminUserManagement />} />
                            <Route path='/admin/quiz-management' element={<AdminQuizManagement />} />
...
function Landing({ user, loading }) {
    ...
    return (
      <>
        <LandingNavBar />
      <div className="BRUNTA h-screen max-h-screen overflow-y-auto ">
        <section class="bg-white dark:bg-gray-900 " id='home'>
    <div class=" max-w-screen-xl  mx-auto text-center " style={{background: 'url("https://images.unsplash.com/photo-1501504905252-473c47e087f8?q=80&w=1374&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D")',backgroundRepeat:'no-repeat',backgroundSize:'cover', backgroundPositionY:'40%'} }>

        <span className='bg-black bg-opacity-40 w-full px-4 py-8 lg:py-16 lg:px-12 block max-w-screen-xl'>

        <h1 class="mb-4 text-4xl font-bold leading-none tracking-tight  md:text-5xl lg:text-6xl text-gray-200"><i><span className='text-blue-600 font-light '>Brunta</span></i> , Your Ultimate GCE Revision App </h1>
        <p class="mb-8 text-lg font-normal text-gray-400 lg:text-xl sm:px-16 xl:px-48 dark:text-gray-400">Discover an extensive library of dynamic MCQs, covering a wide range of subjects for A-Level and O-Level students. Take the quizzes anytime, anywhere, and track your progress as you learn</p>
        <div class="flex flex-col mb-8 space-y-4 lg:mb-16 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-4">
            ...
            <a href="/signup" class="inline-flex items-center justify-center px-5 py-3 text-base font-medium text-center text-white border border-gray-300 rounded-lg hover:bg-gray-100 hover:text-gray-800 focus:ring-4 focus:ring-gray-100  dark:border-gray-700 dark:hover:bg-gray-700 dark:focus:ring-gray-800 ">
                Sign Up
            </a>
        </div>
...
<section class="bg-white dark:bg-gray-900">
  <div class="max-w-screen-xl px-4 py-8 mx-auto sm:py-16 lg:px-6">
      <div class="max-w-screen-md mb-8 lg:mb-16">
          <h2 class="mb-4 text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">Built to UP! your grades, Brunta features : </h2>
                </div>
      <div class="space-y-8 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-12 md:space-y-0">
      <div class="p-6 bg-white rounded-xl shadow-md flex flex-col justify-between dark:bg-gray-900">
              <div class="mb-4">
                  <svg class="w-8 h-8 text-blue-600 dark:text-blue-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.532 1.532 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"></path></svg>
              </div>
              <h3 class="mb-2 text-xl font-bold dark:text-white">Extensive Library of Dynamic MCQs</h3>
              <p class="text-gray-500 dark:text-gray-400">Covering a wide range of subjects for A-Level and O-Level students.</p>
          </div>
          <div class="p-6 bg-white rounded-xl shadow-md flex flex-col justify-between dark:bg-gray-900">
              <div class="mb-4">
                  <svg class="w-8 h-8 text-blue-600 dark:text-blue-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.532 1.532 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"></path></svg>
              </div>
              <h3 class="mb-2 text-xl font-bold dark:text-white">Personalized Revision Plans</h3>
              <p class="text-gray-500 dark:text-gray-400">Tailored to your learning pace and progress.</p>
          </div>
          <div class="p-6 bg-white rounded-xl shadow-md flex flex-col justify-between dark:bg-gray-900">
              <div class="mb-4">
                  <svg class="w-8 h-8 text-blue-600 dark:text-blue-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.532 1.532 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"></path></svg>
              </div>
              <h3 class="mb-2 text-xl font-bold dark:text-white">Detailed Performance Tracking</h3>
              <p class="text-gray-500 dark:text-gray-400">Identify your strengths and weaknesses by subject.</p>
          </div>
          <div class="p-6 bg-white rounded-xl shadow-md flex flex-col justify-between dark:bg-gray-900">
              <div class="mb-4">
                  <svg class="w-8 h-8 text-blue-600 dark:text-blue-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.532 1.532 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"></path></svg>
              </div>
              <h3 class="mb-2 text-xl font-bold dark:text-white">Interactive Revision Tools</h3>
              <p class="text-gray-500 dark:text-gray-400">Enhance understanding and retention.</p>
          </div>
          <div class="p-6 bg-white rounded-xl shadow-md flex flex-col justify-between dark:bg-gray-900">
              <div class="mb-4">
                  <svg class="w-8 h-8 text-blue-600 dark:text-blue-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.532 1.532 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"></path></svg>
              </div>
              <h3 class="mb-2 text-xl font-bold dark:text-white">Flexible Quiz Modes</h3>
              <p class="text-gray-500 dark:text-gray-400">Customized practice: Random, Paced, Multi-Subject.</p>
          </div>
          <div class="p-6 bg-white rounded-xl shadow-md flex flex-col justify-between dark:bg-gray-900">
              <div class="mb-4">
                  <svg class="w-8 h-8 text-blue-600 dark:text-blue-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.532 1.532 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"></path></svg>
              </div>
              <h3 class="mb-2 text-xl font-bold dark:text-white">Competitive Environment</h3>
              <p class="text-gray-500 dark:text-gray-400">Compete with others in engaging Vs. Mode (future feature).</p>
          </div>
...
<section class="bg-white dark:bg-gray-900">
    <div class="items-center max-w-screen-xl gap-16 px-4 py-8 mx-auto lg:grid lg:grid-cols-2 lg:py-16 lg:px-6">
        <div class="text-gray-500 sm:text-lg dark:text-gray-400">
            <h2 class="mb-4 text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">We didn't reinvent the wheel, but we built a better engine for learning.</h2>
            ...
            <p>We will launch the full and open version of Brunta on August 1st, 2025.</p>
        </div>
        <div class="grid grid-cols-2 gap-4 mt-8">
            <img class="w-full rounded-lg" src="https://images.unsplash.com/photo-1576267423445-b2e0074d68a4?w=400&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTl8fGdyb3VwJTIwb2YlMjBmcmllbmR8ZW58MHx8MHx8fDA%3D" alt="office content 1"></img>

            <img class="w-full  mt-4 rounded-lg lg:mt-10" src='https://plus.unsplash.com/premium_photo-1706759227882-56a909c0369c?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D' alt="office content 2"></img>
            </div>
    </div>
</section>
...
<section class="bg-white dark:bg-gray-900">
    <div class="items-center max-w-screen-xl gap-8 px-4 py-8 mx-auto xl:gap-16 md:grid md:grid-cols-2 sm:py-16 lg:px-6">
        ...
        <div class="mt-4 md:mt-0">
            <h2 class="mb-4 text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white">Let's grow our grades and IQ together.</h2>
            <p class="mb-6 text-gray-500 md:text-lg dark:text-gray-400">Brunta helps you to easily and efficiently study revise for your O and A Level GCE exams from the comfort of your phone or any device with access to the internet.</p>
            <a href="/login" class="inline-flex items-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:focus:ring-blue-900">
                Get started
...
<footer class="p-4 bg-white sm:p-6 dark:bg-gray-800">
    <div class="max-w-screen-xl mx-auto">
        <div class="md:flex md:justify-between">
            <div class="mb-6 md:mb-0">
                <a href="/" class="flex items-center">
                    ...
                    <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Brunta</span>
                </a>
            </div>
        ...
        <div class="sm:flex sm:items-center sm:justify-between mb-20">
            <span class="text-sm text-gray-500 sm:text-center dark:text-gray-400">© 2024 <a href="/" class="hover:underline">Brunta™</a>. All Rights Reserved.
            </span>
